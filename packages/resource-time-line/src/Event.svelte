<script>
    ...
    let {displayEventEnd, eventBackgroundColor, eventTextColor,eventColor, eventContent, eventClick, eventDidMount,
        eventClassNames, eventMouseEnter, eventMouseLeave, slotEventOverlap, slotDuration, slotWidth, theme,
    ...
    $: {
        ...
        let top = (start - offset) / step * $slotWidth;
        let height = (end - start) / step * $slotWidth;
        let maxHeight = ($_slotTimeLimits.max.seconds / 60 - start) / step * $slotWidth;
        ...
        if (!bgEvent(display) && !helperEvent(display) || ghostEvent(display)) {
            style +=
                ...
                `height:${100 / chunk.group.columns.length * ($slotEventOverlap ? 0.5 * (1 + chunk.group.columns.length - chunk.column) : 1)}%;`
            ;
        }
